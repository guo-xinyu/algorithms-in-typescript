...
const log4jsLogger = log4js.getLogger('unitTest');
...
//* *********************************************************
type Level = Record<LevelCode, LevelConfig>;

type LogFunction = (param: string) => void;
...
const Level: Level = {
  TRACE: {
    priority: 0,
    outputString: LevelCode.TRACE
  },
  DEBUG: {
    priority: 100,
    outputString: LevelCode.DEBUG
  },
  INFO: {
    priority: 200,
    outputString: LevelCode.INFO
  },
  WARN: {
    priority: 300,
    outputString: LevelCode.WARN
  },
  ERROR: {
    priority: 400,
    outputString: LevelCode.ERROR
  },
  FATAL: {
    priority: 500,
    outputString: LevelCode.FATAL
  },
  OFF: {
    priority: 1000,
    outputString: LevelCode.OFF
  }
};
 ...
 */
function setLogLevel(newLevel: LevelConfig): void {
  logLevel = newLevel;
}
 ...
 */
function logMessage(computedMessage: string): void {
 ...
 *
 * @returns {string} computedMessage - the actual computed message (complete with
 ...
 */
function log(messageLogLevel: LevelConfig, message: string, source?: string,
  logFunction?: (param: string) => void): string {
  let computedMessage = '';
  if (messageLogLevel.priority >= logLevel.priority) {
    ...
    //   message;
    computedMessage = ((source) ? source + ': ' : '') + message;
 ...
 * the current log level at the moment this function was called.
 */
function trace(message: string, source?: string, logFunction?: LogFunction): string {
 ...
 * the current log level at the moment this function was called.
 */
function debug(message: string, source?: string, logFunction?: LogFunction): string {
 ...
 * the current log level at the moment this function was called.
 */
function info(message: string, source?: string, logFunction?: LogFunction): string {
 ...
 * the current log level at the moment this function was called.
 */
function warn(message: string, source?: string, logFunction?: LogFunction): string {
 ...
 * the current log level at the moment this function was called.
 */
function error(message: string, source?: string, logFunction?: LogFunction): string {
 ...
 * the current log level at the moment this function was called.
 */
function fatal(message: string, source?: string, logFunction?: LogFunction): string {
